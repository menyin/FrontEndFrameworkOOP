(function (global, doc) {
    ...
    var hbjs = global.hbjs = {
        ...
        html = doc.documentElement, head = doc.head || doc.getElementsByTagName("head")[0],
        baseElement = head.getElementsByTagName("base")[0], charset = 'utf-8', isDebug = false, regReady = /(loaded|complete)/i,
        preloadMods = [],
        globalMods = [],
        preComboMods = {},
        comboMods = {},
        ...
        count: '@count:',
        combo: '@combo:'
    }, indexOf = Array.prototype.indexOf, filter = Array.prototype.filter, isObject = hbjs.isObject = isType("Object"), isString = hbjs.isString = isType("String"), isArray = hbjs.isArray = Array.isArray || isType("Array"), isFunction = hbjs.isFunction = isType("Function"), isBoolean = hbjs.isBoolean = isType("Boolean"), array = hbjs.arrayUtil = {
    ...
    function loadJS(path, callback, isAsync) {
        var module = ('' + path).split('::'), modulePath = module[0], none;
    ...
    function takeGlobalModule(moduleName, isArray) {
        var mods = isArray ? [] : {}, type = isArray ? moduleType.init : moduleType.module, cache = package['#module:cache'], deplist = cache[type + moduleName].depend, status = true;
    ...
    function checkDefinedLoaded(moduleName) {
        var cache = package['#module:cache'], status = cache[moduleType.loaded + moduleName] || checkDefinedList(moduleName), factory, result;
        if (status) {
            if (factory = cache[moduleType.module + moduleName].factory) {
                ...
                if (!result && !isBoolean(result)) {
                    comboExports[moduleName] = cache[moduleType.factory + moduleName] = factory.apply(hbjs, args) || factory;
                }
    ...
    function checkDefinedList(moduleName) {
        var cache = package['#module:cache'], module = cache[moduleType.module + moduleName], deplist = module.depend;
    ...
    function getUseFactorys(moduleName, id) {
        var cache = package['#module:cache'], factorys = cache[moduleType.initList + moduleName];
        if (!factorys) {
        ...
        cache[moduleType.initList + moduleName][id] = cache[moduleType.initList + moduleName][id] || cache[moduleType.init + moduleName + id].factory;
    ...
    function checkLoaded(moduleName, id) {
        var cache = package['#module:cache'], status = cache[moduleType.loaded + moduleName], factorys = getUseFactorys(moduleName, id);
        if (!factorys) {
            return;
        }
        if (!status) {
            var moduleNames = cache[moduleType.init + moduleName], i = 0, deplist;
    ...
    function normailzeVer(key) {
        if (normailzeNames) {
            var ext = '.js', lastIndex = key.lastIndexOf('.') + 1;
    ...
    var package = {
        ...
        '#module:getCombosPath': function (module) {
            ...
            var dependPaths = module.depend, moduleName = module.module, path = "", val, version = 0;
        ...
        require: function (module) {
            var cache = this['#module:cache'], moduleName = module['module'], initId = module.initId;
            if (initId != undefined) {
        ...
        define: function (module, deplist, factory) {
            ...
            package.require({module: module, depend: deplist, factory: factory});
        },
        use: function (deplist, factory) {
            ...
            var moduleName = deplist.join('|kk|'), cache = this['#module:cache'];
        ...
        if (!path) {
            ...
            if (!result) {
                var scripts = doc.getElementsByTagName("script"), reg = /hbjs.js(\W|$)/i, src;
    ...
    hbjs.config({
        ...
        map: {jquery: 'jquery.min', uploadify: 'cqjob/uploadify/uploadify'}
    });
...
(function (a, b) {
    ...
    })/*,a.jQuery=a.$=p,typeof define=="function"&&define.amd&&define.amd.jQuery&&define("jquery",[],function(){return p})*/, typeof define == "function" && define("jquery", function () {
        return p
    })
})(window);
;
define('base.util', function (require, exports) {
    ...
    var hasEnumBug = !({toString: 1}.propertyIsEnumerable('toString')), enumProperties = ['constructor', 'hasOwnProperty', 'isPrototypeOf', 'propertyIsEnumerable', 'toString', 'toLocaleString', 'valueOf'], encode = encodeURIComponent, decode = decodeURIComponent, reg_trim = /^\s+|\s+$/g, reg_arr_key = /^(\w+)\[\]$/, slice = Array.prototype.slice, toString = Object.prototype.toString, doc = document, elementStyle = doc.createElement('div').style;
    ...
    exports.type = {
        ...
        isPlainObject: function (o) {
            return o && this.isObject(o) && !o['noteType'] && !this.isWindow(o);
        },
        isEmptyObject: function (o) {
            if (!o || !this.isPlainObject(o) || o.nodeType || this.isWindow(o) || !o.hasOwnProperty) {
                return false;
            }
            for (var p in o) {
        ...
        isEmptyAttrValue: function (o) {
            return o == null || (this.isString(o) || this.isArray(o)) && o.length === 0 || this.isEmptyObject(o);
        },
        isEmptyString: function (s, rev) {
            var len = exports.string.trim(s).length, rev = rev ? len > 0 : len === 0;
    ...
    exports.extend = function (t, s) {
        if (!s || !t)
            return t;
        var obj = Object.prototype, O = function (o) {
            function F() {
            };
            F.prototype = o;
            return new F();
        }, sp = s.prototype, op = O(sp);
        t.prototype = op;
        op.constructor = t;
        t.superclass = sp;
    ...
    exports.bind = function () {
        var args = slice.call(arguments) || [], fun = args.shift(), obj = args.shift();
        return function (e) {
            var ar = slice.call(arguments) || null;
            return exports.type.isFunction(fun) && fun.apply(obj, ar.concat(args));
        }
    };
...
;
define('base.shape', ['base.event', 'base.aspect', 'base.attribute', 'base.util', 'base.class', 'jquery'], function (require, exports, module) {
    var util = require('base.util'), aspect = require('base.aspect'), attribute = require('base.attribute'), events = module['base.event'], $ = module['jquery'], Class = require('base.class').Class, cidCounter = 0, cachedInstances = [], shape = Class(function (config) {
        this.cid = uniqueCid();
...
define('base.event', 'base.util', function (require, exports) {
    var util = require('base.util'), eventSplitter = /\s+/, eventDispatcher = {
        on: function (type, callback, context) {
            ...
            if (!this._listeners || !this._listeners[type])
                return;
            var list = this._listeners[type], pass = false;
...
define('base.aspect', function (require, exports) {
    ...
    function aop(status, methodName, callback) {
        var names = methodName.split(eventSplitter), name, method;
    ...
    function wrap(methodName) {
        ...
        this[methodName] = function () {
            var args = Array.prototype.slice.call(arguments), beforeArgs = ['before:' + methodName].concat(args);
            if (!this.trigger.apply(this, beforeArgs));
            var ret = method.apply(this, arguments), afterArgs = ['after:' + methodName].concat(args);
...
define('base.attribute', 'base.util', function (require, exports) {
    var util = require('base.util'), cached;
    exports.initAttrs = function (config, instance) {
        var attrs = this.attrs = {}, readOnlyAttrs = this.readOnlyAttrs || {}, specialProps = this.propsInAttrs || [], cached = instance;
    ...
    exports.set = function (key, val, options) {
        ...
        var options = options || {}, override = options.override, readOnly = !!options.readyOnly, now = this.attrs, read = this.readOnlyAttrs;
        for (key in attrs) {
            if (!attrs.hasOwnProperty(key))continue;
            var attr = now[key] || (now[key] = {}), val = attrs[key];
            if (read[key]) {
    ...
    function mergeInheritedAttrs(attrs, read, instance, specialProps) {
        var inherited = [], readOnlys = [], proto = instance.constructor.prototype;
        while (proto) {
...
define('tools.cookie', function (require, exports) {
    var util = require('base.util'), doc = document, decode = decodeURIComponent, encode = encodeURIComponent, config = {
        isDecode: true,
        isJSON: true
    };
    ...
    exports.set = function (name, value, options) {
        ...
        var text = name + "=" + value, date = expires;
    ...
    var parseCookie = exports.parseCookie = function (text, options) {
        ...
        if (!util.type.isEmptyString(text)) {
            var cookie, cookieName, cookieValue, value, cookies = text.split(/;\s/g);
